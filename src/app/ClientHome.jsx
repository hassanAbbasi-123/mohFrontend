...
import { useSearchParams } from "next/navigation"; // Added for search params support
...
const ChatBot = ({ isOpen, setIsOpen }) => {
  ...
  const getAIResponse = (userMessage) => {
    const username = process.env.NEXT_PUBLIC_WEBHOOK_USER || '';
    const password = process.env.NEXT_PUBLIC_WEBHOOK_PASS || '';
    const credentials = `${username}:${password}`;
    const authHeader = `Basic ${btoa(credentials)}`; // Fixed: Use btoa() for browser compatibility
        ...
        >
          {!isMinimized && (
            <>
              ...
              {/* FAQs Section - Only shows when Ask Any Question button clicked */}
...
// Floating Chat Button Component (unchanged)
...
// Main Home Component (updated with optional search params)
export default function Home() {
  const searchParams = useSearchParams(); // Added for search params
  ...
  return (
    <CartProvider>
      <main className="bg-gray-50 min-h-screen flex flex-col">
        ...
        {/* Optional: If using HomePageClient with search params, uncomment below */}
