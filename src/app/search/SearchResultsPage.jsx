          ...
          <div className="flex flex-col lg:flex-row gap-6 items-start lg:items-center justify-between">
            <div className="flex-1 min-w-0">
              ...
              <p className="text-gray-600 text-lg">
                Found {displayedProducts.length} results for "<span className="font-semibold text-gray-900">{searchQuery}</span>"
              </p>
            </div>
                ...
                <input
                  ...
                  onChange={(e) => {
                    ...
                    if (e.target.value.trim()) {
                      router.push(`/search?query=${encodeURIComponent(e.target.value.trim())}`);
                    } else {
                      router.push(`/search`);
                    }
                  }}
                  className="w-full pl-12 pr-4 py-3.5 bg-white/80 backdrop-blur-sm border border-gray-300/80 rounded-2xl focus:ring-3 focus:ring-blue-500/20 focus:border-blue-500 transition-all duration-200 shadow-sm hover:shadow-md"
                />
              </div>
          ...
          <section className="lg:col-span-3">
            {displayedProducts.length === 0 ? (
              <div className="bg-white/80 backdrop-blur-lg rounded-3xl shadow-2xl p-16 text-center border border-gray-200/60">
                ...
                <p className="text-gray-600 text-lg mb-8">Try adjusting your search terms or filters to find what you're looking for</p>
                <button
                  onClick={() => {
                    setCategoryFilter("");
                    setMinPrice("");
                ...
                {displayedProducts.map((product) => (
                  ...
                  >
                    ...
                    <div className="relative h-72 bg-gradient-to-br from-gray-50 to-gray-100 overflow-hidden">
                      ...
                      {/* Stock Status Badge */}
                      {product.inStock && (
                        <div className="absolute top-4 left-4 bg-gradient-to-r from-green-500 to-emerald-600 text-white text-xs font-semibold px-3 py-1.5 rounded-full shadow-lg">
                          In Stock
                        </div>
                      )}
