...
const SellerProducts = () => {
  ...
  // Optimized image URL builder (handles relative paths + Cloudinary optimizations)
  const optimizeCloudinaryImage = (url, width = 600, height = 400) => {
    if (!url || typeof url !== 'string') return '/placeholder.jpg';
    
    let processedUrl = url.trim();

    // Convert relative paths to absolute using API base
    if (!processedUrl.startsWith('http')) {
      processedUrl = `${API_BASE}/${processedUrl.replace(/^\/+/, '')}`;
    }

    // If it's already a Cloudinary URL with transformations, return as-is
    if (processedUrl.includes('upload/') && processedUrl.includes('cloudinary.com')) {
      // Check if already has transformations
      if (processedUrl.includes('/upload/w_')) {
        return processedUrl;
      }
  ...
  // Safely extract categories array (handles direct array or wrapped { categories: [...] } / { data: [...] })
  ...
  // Safely extract products array (handles direct array or wrapped { products: [...] } / { data: [...] })
